...
// Círculo aleatorio en canvasRandom (sin que se salga el borde)
let borde = 5; // Grosor del borde
let randomRadius = Math.floor(Math.random() * 50 + 20);
let randomX = Math.random() * (canvasRandom.width - 2 * (randomRadius + borde)) + (randomRadius + borde);
let randomY = Math.random() * (canvasRandom.height - 2 * (randomRadius + borde)) + (randomRadius + borde);

let miCirculoRandom = new Circle(randomX, randomY, randomRadius, "green", "Tec", "rgb(83, 186, 52)");
miCirculoRandom.draw(ctxRandom);

// Círculos múltiples en canvasMultiple (sin que se salgan)
let arrayCircle = [];
for (let i = 0; i < 10; i++) {
  let randomRadius = Math.floor(Math.random() * 10 + 20);
  let randomX = Math.random() * (canvasMultiple.width - 2 * (randomRadius + borde)) + (randomRadius + borde);
  let randomY = Math.random() * (canvasMultiple.height - 2 * (randomRadius + borde)) + (randomRadius + borde);
